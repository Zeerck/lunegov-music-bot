...
class Playlist:
    """Сохраняет ссылки YouTube на треки, которые будут воспроизведены и уже воспроизведены, и предлагает базовые операции с очередями."""
    ...
    def remove(self, index: int) -> Song:
        if index < 0:
            raise PlaylistError("Отрицательные индексы не поддерживаются.")
        if index == 0:
            raise PlaylistError(
                "Невозможно удалить первый трек, так как он уже воспроизводится."
            )
        try:
            song = self.playque[index]
        except IndexError as e:
            raise PlaylistError("Неверная позиция.") from e
    ...
    def move(self, oldindex: int, newindex: int):
        ...
        if oldindex == 0 or newindex == 0:
            raise PlaylistError(
                "Невозможно переместить первый трек, так как он уже воспроизводится."
            )
        try:
    ...
    def queue_embed(self) -> Embed:
        embed = Embed(
            title=":scroll: Очередь [{}]".format(len(self.playque)),
