...
class GuildSettings(Base):
    ...
    async def load(cls, bot: "MusicBot", guild: discord.Guild) -> "GuildSettings":
        "Загрузите объект из базы данных или создайте новый и зафиксируйте его"
    ...
    async def load_many(
        ...
        """Загрузить список объектов из базы данных и создать новые, если они не найдены.
        Возвращает dict с гильдиями в качестве ключей и их настройками в качестве значений"""
    ...
    def format(self, ctx: "Context"):
        ...
        embed.set_footer(
            text="Использование: {}set setting_name value".format(config.BOT_PREFIX)
        )
        ...
        for key in DEFAULT_CONFIG.keys():
            ...
            if not getattr(self, key):
                ...
                embed.add_field(
                    name=key, value=vc.name if vc else "Недоступный голосовой канал", inline=False
                )
                continue
                ...
                embed.add_field(
                    name=key,
                    value=chan.name if chan else "Недоступный канал",
                    inline=False,
                )
                continue
    ...
    async def set_command_channel(self, setting, value, ctx):
        ...
        if not chan:
            await ctx.send(
                "`Ошибка: Канал не найден`\nИспользование: {}set {} channel\nДругие опции: unset".format(
    ...
    async def set_start_voice_channel(self, setting, value, ctx):
        ...
        try:
            ...
            await ctx.send(
                "`Ошибка: Голосовой канал не найден`\nИспользование: {}set {} vchannel\nДругие опции: unset".format(
                    config.BOT_PREFIX, setting
                )
            )
            return False
    ...
    async def set_user_must_be_in_vc(self, setting, value, ctx):
        if value.lower() == "true":
            ...
            await ctx.send(
                "`Ошибка: Значение должно быть True/False`\nИспользование: {}set {} True/False".format(
                    config.BOT_PREFIX, setting
                )
            )
            return False
    ...
    async def set_button_emote(self, setting, value, ctx):
        if not config.ENABLE_BUTTON_PLUGIN:
            await ctx.send("`Ошибка: Плагин кнопки отключен`")
            return False
        ...
        if emoji is None:
            await ctx.send(
                "`Ошибка: Недопустимая эмоция`\nИспользование: {}set {} emote\nДругие опции: unset".format(
                    config.BOT_PREFIX, setting
                )
            )
            return False
    ...
    async def set_vc_timeout(self, setting, value, ctx):
        ...
        if not config.ALLOW_VC_TIMEOUT_EDIT:
            await ctx.send("`Ошибка: Это значение нельзя изменить`")
            return False
...
def run_migrations(connection):
    "Автоматически создает или удаляет таблицы и столбцы, отражая изменения кода."
...
async def extract_legacy_settings(bot: "MusicBot"):
    "Загрузить настройки из устаревшего файла json в БД"
